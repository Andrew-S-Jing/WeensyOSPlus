    ...
    while (heap_top != stack_bottom) {
        ...
        if (x < 2) {
            ...
            for (unsigned long* l = (unsigned long*) heap_top;
                 l != (unsigned long*) (heap_top + ALLOC_PAGES * PAGESIZE);
                 ++l) {
                assert(*l == 0);
            }
            // check we can write to new pages
            for (int i = 0; i < ALLOC_PAGES; ++i) {
                heap_top[i * PAGESIZE] = speed;
            }
            // check we can write to console
            console[CPOS(24, 79)] = speed;
            // record data written
            unsigned pn = (heap_top - heap_bottom) / PAGESIZE;
            for (unsigned i = pn; i < pn + ALLOC_PAGES; ++i) {
                if (i < sizeof(pagemark)) pagemark[i] = speed;
            }
            // update `heap_top`
            heap_top += ALLOC_PAGES * PAGESIZE;
